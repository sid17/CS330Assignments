    ...
    if (exception != NoException)
     {
      ...
      machine->RaiseException(exception, addr);
      return FALSE;
        //printf("Fuck Yeah !!!%c",machine->mainMemory[physicalAddress]);
   //return TRUE;
      }
...
// from the virtual address
    ...
    *physAddr = pageFrame * PageSize + offset;
    ...
    //fifoQueue->append((void *) *physAddr);        // Satvik

    //pg=lru_algo->tail->val;
//printf("Translate called\n");

    if (pageReplacementAlgo==3)
    {
      ASSERT(lru_arr[pageFrame]->val<NumPhysPages);
    if (mainMemoryStatus[pageFrame]!=-1)
    {
     // printf("Translate called\n");
      deleted(lru_algo,lru_arr[pageFrame]);
      insertd(lru_algo,lru_arr[pageFrame]);
    }
    }
