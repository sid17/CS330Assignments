...
//printf("Assertion failed due to page fault: %d .. %d , %d  \n",addr,pageNumber,code);
...
ASSERT(pageNumber>=0);
 ...
 if (pageTable[virtualPageNumber].readFromDisk==FALSE)
  ...
  //printf("Initialized code %d %d %d\n",vpn_code,offset_code,size_code);
  }
  ...
  //printf("Initialized data %d %d %d\n",vpn_data,offset_data,size_data);
    }
     
int arr_size=noffH.code.size + noffH.initData.size + noffH.uninitData.size 
      + UserStackSize;




arr_size = divRoundUp(arr_size, PageSize) * PageSize;

//printf("Size and pageNumber %d,%d\n",divRoundUp(arr_size, PageSize) * PageSize,pageNumber);

char * val=new char[arr_size];
bzero(val,arr_size);
execFile->ReadAt(&(val[vpn_code * PageSize + offset_code]),
noffH.code.size, noffH.code.inFileAddr);
execFile->ReadAt(&(val[vpn_data * PageSize + offset_data]),
noffH.initData.size, noffH.initData.inFileAddr);
int l=virtualPageNumber*PageSize;
//printf("\n------------\n");
while(l<virtualPageNumber*PageSize+PageSize)
{
  machine->mainMemory[pageNumber*PageSize+l-virtualPageNumber*PageSize]=val[l];
...
//pageTable[virtualPageNumber].use = FALSE;
//pageTable[virtualPageNumber].dirty = FALSE;
//pageTable[virtualPageNumber].readOnly = FALSE;    // if the code segment was entirely on
//pageTable[virtualPageNumber].shared=FALSE; 
//pageTable[virtualPageNumber].readFromDisk=FALSE; 

}
else
{
  //printf("Get info from the buffer \n");
...
//pageTable[virtualPageNumber].readOnly = FALSE;    // if the code segment was entirely on
//pageTable[virtualPageNumber].shared=FALSE; 
//pageTable[virtualPageNumber].readFromDisk=TRUE; 
