...
Semaphore *semArray[MAX_SEMAPHORE_COUNT];

int semValid[MAX_SEMAPHORE_COUNT];

int semKey[MAX_SEMAPHORE_COUNT];

int pageReplacementAlgo;

// List *fifoQueue;     // Satvik


listpt fifoQueue;

nodept fifo_arr[NumPhysPages];



listpt lru_algo;
                        //allocate the space for empty list
nodept lru_arr[NumPhysPages];




int arr_LruClock[NumPhysPages];
int * Lru_Start_Pointer;  // Satvik


int NumPageFaults;


Condition *condArray[MAX_COND_COUNT];

int condValid[MAX_COND_COUNT];

int condKey[MAX_COND_COUNT];
...
Initialize(int argc, char **argv)
{
    ...
    pageReplacementAlgo=RANDOM; // Default
    ...
    for (i=0; i<MAX_THREAD_COUNT; i++) { threadArray[i] = NULL; exitThreadArray[i] = false; completionTimeArray[i] = -1; }
     ...
     for (i=0; i<MAX_SEMAPHORE_COUNT; i++) { semArray[i] = NULL;semKey[i]=-1;semValid[i] = 0; }


     for (i=0; i<MAX_COND_COUNT; i++) { condArray[i] = NULL;condKey[i]=-1;condValid[i] = 0; }
      ...
      for (i=0;i<NumPhysPages;i++)
        {
            //fprintf(stderr, "%d,",i);
            lru_arr[i]=createnode(i);
            //fprintf(stderr, "%d,",lru_arr[i]->val);
        }
        lru_algo=new dllist();
        ...
        {
            //fprintf(stderr, "%d,",i);
            fifo_arr[i]=createnode(i);
            //fprintf(stderr, "%d,",lru_arr[i]->val);
        }
        fifoQueue=new dllist();   
    
    

    Lru_Start_Pointer = new int;
    *Lru_Start_Pointer=0;
    for (i=0;i<NumPhysPages;i++)
     {
        arr_LruClock[i]= 0;
    ...
    // insertd(lru_algo,lru_arr[0]);
    // insertd(lru_algo,lru_arr[1]);
    // insertd(lru_algo,lru_arr[3]);
    // insertd(lru_algo,lru_arr[4]);
    // deleted(lru_algo,lru_arr[3]);
    // insertd(lru_algo,lru_arr[3]);
    // deleted(lru_algo,lru_algo->tail);
    // printlist(lru_algo);


    //fifoQueue = new List;     // Satvik    
