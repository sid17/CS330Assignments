...
extern listpt lru_algo;
                        //allocate the space for empty list
extern nodept lru_arr[NumPhysPages];


extern int arr_LruClock[NumPhysPages];
extern int * Lru_Start_Pointer;  // Satvik

//extern List *fifoQueue;


extern listpt fifoQueue;
                        //allocate the space for empty list
extern nodept fifo_arr[NumPhysPages];



extern Semaphore *semArray[MAX_SEMAPHORE_COUNT];

extern int semKey[MAX_SEMAPHORE_COUNT];

extern int semValid[MAX_SEMAPHORE_COUNT];
...
extern Condition *condArray[MAX_COND_COUNT];

extern int condKey[MAX_COND_COUNT];

extern int condValid[MAX_COND_COUNT];
